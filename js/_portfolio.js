...
async function claimCardsRefresh(){
	...
	if(MyLibrary.claimsource ==1){//wallet
		//Holdings
		$('#sect_claims_w').empty().append(MyLibrary.TokensBalance+' Tokens');
		//Claims To Date
		try{
...
async function port_staticView(){
	...
	try{
		...
		$('#mbcard_taxstatus').empty().append('<p class="markgreen">'+sellRate+'% Sell Tax Today</p>');
	}catch(error) {
		console.log(error); 
	}
...
async function averageCost(){
	...
	if(balance > 0){//hate seeing the revert error so only if the user bought some(not perfect)
		try{
			var asv = await tokenInst.methods.jeetValue(MyLibrary.wallet).call();
			var asv = web3.utils.fromWei(asv[0], "ether");
			var asv = Number(asv).toFixed(12);

			var sellAmount = (asv[1] / Math.pow(10, MyLibrary.decimals)).toFixed(2);
